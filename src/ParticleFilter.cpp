...
ParticleFilter::ParticleFilter(int num_particles) : num_particles(num_particles), gen(rd()) {}

void ParticleFilter::init(double x, double y, double std[]) {
...
void ParticleFilter::predict(double delta_t, double std_pos[], double delta_x, double delta_y) {
...
double ParticleFilter::distanceToLine(double px, double py, double x1, double y1, double x2, double y2) {
    double A = px - x1;
    double B = py - y1;
    double C = x2 - x1;
    double D = y2 - y1;
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], const Model::PointCloud observations) {
