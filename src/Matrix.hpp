...
class Matrix
{
	public:
		...
		//@}
		/**
		 * @brief Do one Kalman Iteration
		 * @param stateVector contains relevant state information
		 * @param covarianceMatrix contains any process error in a square matrix
		 * @return a pair of matrixes, the first being the new stateVector, and the second being the new covarianceMatrix
		*/
		static std::pair<Matrix<double, M, 1>, Matrix<double, M, M>> doKalmanIteration(Matrix<double, M, 1> stateVector, Matrix<double, M, M> covarianceMatrix, 
